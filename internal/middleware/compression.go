...
type compressWriter struct {
	...
	wroteHeader     bool
	shouldCompress  bool // Флаг для кэширования решения
}
...
func (c *compressWriter) Write(p []byte) (int, error) {
	if !c.wroteHeader {
		c.WriteHeader(http.StatusOK) // Автоматический вызов при первом Write
	}
...
func (c *compressWriter) WriteHeader(statusCode int) {
	...
	// Принимаем решение о сжатии ДО отправки заголовков
	if statusCode < 300 {
...
// Проверяем необходимость сжатия по Content-Type
